<template>
  <div class="flex h-screen bg-gray-950 text-gray-100 font-mono overflow-hidden">
    ...
    <!-- ═══════════════════════════════════════════════════════════
         LEFT SIDEBAR — File Upload & Status (SRS 3.1)
    ═══════════════════════════════════════════════════════════ -->
    <aside class="w-72 min-w-[280px] bg-gray-900 border-r border-gray-800 flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto px-4 py-4">
        ...
        <button
          ...
          class="mt-4 w-full text-xs text-red-400 border border-red-900 rounded px-3 py-1.5 hover:bg-red-900/30 transition-colors"
          @click="clearStore"
        >
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
      ...
      <!-- Messages (SRS 3.1: chat history, session-only) -->
      <div ref="messagesContainer" class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        ...
        >
          ...
          >
            <!-- SRS 3.1: Markdown rendering with code syntax highlighting -->
            <div
      ...
      <!-- ── Input Area (SRS 3.1 + SRS 5.1: disable while generating) ── -->
      <div class="bg-gray-900 border-t border-gray-800 px-6 py-4 shrink-0">
        <div class="flex gap-3 items-end">
          <textarea
            ...
            @keydown.shift.enter="() => {}"
          ></textarea>
          <!-- SRS 5.1: Button disabled while generating to prevent spam -->
          <button
...
// Configure marked with highlight.js for code blocks (SRS 3.1)
marked.setOptions({
  highlight(code, lang) {
...
// ── Reactive state ───────────────────────────────────────────────────────────
const messages = reactive([])          // Chat history (session-only per SRS 3.1)
const inputMessage = ref('')
const isGenerating = ref(false)        // SRS 5.1: controls send button disable
...
function handleServerMessage(data) {
  // SRS 4.2: { "token": "...", "status": "streaming" }
  ...
  // SRS 5.1: rate limit or other error
...
async function sendMessage() {
  ...
  isGenerating.value = true   // SRS 5.1: disable send button
  scrollToBottom()

  // SRS 4.2: Client → Server message format
...
async function uploadFile(file) {
  ...
  try {
    ...
    uploadStatus.value = { type: 'error', message: 'Cannot reach backend. Is FastAPI running?' }
  }
...
async function clearStore() {
  try {
    await fetch(`${API_URL}/clear-store`, { method: 'DELETE' })
...
/* ── Markdown prose styles inside chat bubbles ────────────────────────────── */
.prose pre {
